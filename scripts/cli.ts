...
async function initSubmodules(skipPrompt = false) {
  ...
  const existingProjects = allProjects.filter((p) =>
    p.type === "llms"
      ? existsSync(join(root, p.path))
      : submoduleExists(p.path),
  );
...
async function downloadLlms(name: string, url: string) {
  ...
  const links = [
    ...content.matchAll(/\[.*?\]\((.*?llms.*?\.(?:txt|mdx?))\)/gi),
  ];
  ...
  for (const match of links) {
    ...
    try {
      ...
      console.error(
        `Failed to download linked LLMS file ${linkUrl.href}: ${e}`,
      );
    }
  }
...
async function cleanup(skipPrompt = false) {
  ...
  if (extraSubmodules.length > 0 || extraLlms.length > 0) {
    if (extraSubmodules.length > 0) {
      p.log.warn(
        `Found ${extraSubmodules.length} submodule(s) not in meta.ts:`,
      );
      for (const path of extraSubmodules) {
        p.log.message(`  - ${path}`);
      }
    ...
    options: [
      {
        value: "sync",
        label: "Sync projects",
        hint: "Pull latest and sync Type 2 & 4 skills",
      },
      {
        value: "init",
        label: "Init projects",
        hint: "Add new submodules or LLMS",
      },
      {
        value: "check",
        label: "Check updates",
        hint: "See available updates (git only)",
      },
      {
        value: "cleanup",
        label: "Cleanup",
        hint: "Remove unused projects and skills",
      },
    ],
  });
